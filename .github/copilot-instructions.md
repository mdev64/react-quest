# GitHub Copilot Instructions — ReactQuest
...
**ReactQuest** is a client-side React learning app where users read 15 progressive lessons about React and answer quizzes to unlock the next lesson. It is built with:

- **Vite 8** + **React 19** (functional components, hooks only — no class components)
...
src/
  ...
  components/
    ...
    Quiz.css
    ReactLogo.jsx        # Spinning SVG React logo
  pages/
...
- Theme preference is persisted in `localStorage` under the key `react-quest-theme`.
...
The Vite `base` is set to `/react-quest/` in `vite.config.js` — change this if the repo is renamed.
